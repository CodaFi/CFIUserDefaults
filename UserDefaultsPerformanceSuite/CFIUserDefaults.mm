...
NSString * const CFIUserDefaultsDidChangeNotification = @"CFIUserDefaultsDidChangeNotification";
...
@implementation CFIUserDefaults {
...
+ (instancetype)standardUserDefaults {
    ...
    while (!sharedInstance) {
        ...
        if(OSAtomicCompareAndSwapPtrBarrier(0x0, CFI_SAFEBRIDGE(void *)temp, &sharedInstance)) {
			[CFI_SAFEBRIDGE(CFIUserDefaults*)sharedInstance init];
        }
        else {
            temp = nil;
        }
    }
    return CFI_SAFEBRIDGE(CFIUserDefaults*)(sharedInstance);
#endif
	static dispatch_once_t once;
	static CFIUserDefaults *sharedFoo;
...
+ (void)resetStandardUserDefaults {
	CFIUserDefaults *dbConnection = [self standardUserDefaults];
...
- (void)registerDefaults:(NSDictionary *)registrationDictionary {
	...
	[NSNotificationCenter.defaultCenter postNotificationName:CFIUserDefaultsDidChangeNotification object:nil];
}
...
- (id)init {
	...
	self.path = [[[NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) objectAtIndex:0]stringByAppendingPathComponent:[NSString stringWithFormat:@"Preferences/%@", appBundle]]stringByAppendingPathExtension:@"cfiprefs"];
...
@implementation CFIUserDefaults (CFISubscripting)
